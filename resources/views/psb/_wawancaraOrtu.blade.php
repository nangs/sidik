@if ($psb->calonSiswa->wawancaraOrtu)
<table class="table table-striped table-hover">
    <tbody>
        <tr>
            <th style="width:170px;"> Total Nilai </th>
            <td> : {{ $psb->calonSiswa->nilaiWawancaraOrtu->sum('nilai') }} </td>
        </tr>
        <tr>
            <th> Hasil Wawancara </th>
            <td> : {{ $psb->calonSiswa->wawancaraOrtu->hasil_wawancara ? 'LULUS' : 'DIPERTIMBANGKAN' }} </td>
        </tr>
        <tr>
            <th> Catatan </th>
            <td> : {{ $psb->calonSiswa->wawancaraOrtu->catatan }} </td>
        </tr>
    </tbody>
</table>
...
<table class="table table-striped table-hover">
    ...
    <tbody>
        @foreach($psb->calonSiswa->nilaiWawancaraOrtu as $w)
        <tr>
...
{!! Form::open(['url' => '/wawancara-orang-tua', 'method' => 'POST']) !!}
...
<table class="table table-striped table-hover">
    ...
    <tbody>
        @foreach(\App\FormWawancaraOrangTuaCalonSantri::all() as $i => $p)
        <tr>
